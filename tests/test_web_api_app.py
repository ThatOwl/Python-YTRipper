...
class TestWebApiApp(unittest.TestCase):
    ...
    def test_health_and_session_config_endpoints(self):
        ...
        self.assertIn("No health check completed yet.", index_response)
        self.assertIn("Paths are shown for reference only.", index_response)
        self.assertIn("Paste a video or playlist URL to validate it locally.", index_response)
        self.assertIn("Inspection Guidance", index_response)
        self.assertIn("Local validation runs automatically after a short pause.", index_response)
        self.assertIn("Start Job needs a supported locally validated URL. Inspection is recommended, not required.", index_response)
        ...
        self.assertIn("Loading a preset replaces the in-memory session immediately.", index_response)
        self.assertIn("Preset Guidance", index_response)
        self.assertIn("Loading preset ownership and save behavior...", index_response)
        self.assertIn("Default, custom, and immutable presets will be explained here.", index_response)
