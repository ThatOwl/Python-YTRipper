...
def format_review_candidates(candidates: list[dict[str, Any]], output_format: str = "csv") -> str:
    if output_format == "json":
        return json.dumps(candidates, indent=2, ensure_ascii=False) + "\n"
    return _rows_to_csv(REVIEW_CANDIDATE_FIELDS, candidates)


def format_repair_plans(plans: list[dict[str, Any]], output_format: str = "csv") -> str:
    if output_format == "json":
        return json.dumps(plans, indent=2, ensure_ascii=False) + "\n"
    return _rows_to_csv(REPAIR_PLAN_FIELDS, plans)
